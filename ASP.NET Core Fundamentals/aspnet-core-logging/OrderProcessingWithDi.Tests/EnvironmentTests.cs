...
/// 
/// NOTE: These tests require implementation from previous assignments (Environments).
/// In the template project, endpoints are not implemented yet (only TODO comments).
/// </summary>
public class EnvironmentTests(WebApplicationFactory<Program> factory) : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        // Act
        var response = await this.client.GetAsync("/config/environment");
    ...
    public async Task GetEnvironmentConfig_ReturnsValidEnvironmentName()
    {
        ...
        Assert.True(
            result.EnvironmentName is "Development" or "Staging" or "Production",
    ...
    private class EnvironmentConfigResponse
    {
        ...
        public string? PricingFromEnv { get; init; }
    }
}
