...
public class ReceiptRepositoryTests
{
    [TestCase(1)]
    [TestCase(3)]
    public async Task GetByIdAsync_WhenValidId_ReturnsReceipt(int id)
    {
        ...
        Assert.That(receipt, Is.EqualTo(expected).Using(new ReceiptEqualityComparer()),
            message: "GetByIdAsync method works incorrect");
    }

    [Test]
    public async Task GetAllAsync_WhenCalled_ReturnsAllReceipts()
    {
        await using var context = new TradeMarketDbContext(UnitTestDataHelper.GetUnitTestDbOptions());
        ...
        Assert.That(receipts, Is.EqualTo(ExpectedReceipts).Using(new ReceiptEqualityComparer()),
            message: "GetAllAsync method works incorrect");
    }

    [Test]
    public async Task AddAsync_WhenValidReceipt_AddsReceiptToDatabase()
    {
        await using var context = new TradeMarketDbContext(UnitTestDataHelper.GetUnitTestDbOptions());
    ...
    [Test]
    public async Task DeleteByIdAsync_WhenValidId_DeletesReceiptFromDatabase()
    {
        await using var context = new TradeMarketDbContext(UnitTestDataHelper.GetUnitTestDbOptions());
    ...
    [Test]
    public async Task Update_WhenValidReceipt_UpdatesReceiptInDatabase()
    {
        await using var context = new TradeMarketDbContext(UnitTestDataHelper.GetUnitTestDbOptions());
        ...
        {
            Id = 1, CustomerId = 2, OperationDate = new DateTime(2021, 10, 5), IsCheckedOut = false
        };
    ...
    [Test]
    public async Task GetByIdWithDetailsAsync_WhenValidId_ReturnsReceiptWithIncludedEntities()
    {
        await using var context = new TradeMarketDbContext(UnitTestDataHelper.GetUnitTestDbOptions());
        ...
        {
            Assert.That(receipt,
                Is.EqualTo(expected).Using(new ReceiptEqualityComparer()),
                message: "GetByIdWithDetailsAsync method works incorrect");

            Assert.That(receipt!.ReceiptDetails,
                Is.EqualTo(ExpectedReceiptsDetails.Where(i =>
                {
                    Debug.Assert(expected != null, nameof(expected) + " != null");
                    return i.ReceiptId == expected.Id;
                }).OrderBy(i => i.Id)).Using(new ReceiptDetailEqualityComparer()),
            ...
            Assert.That(receipt.ReceiptDetails!.Select(i => i.Product).OrderBy(i => i.Id),
                Is.EqualTo(ExpectedProducts.Where(i => i.Id == 1 || i.Id == 2)).Using(new ProductEqualityComparer()),
                message: "GetByIdWithDetailsAsync method doesnt't return included entities");

            Assert.That(receipt.ReceiptDetails!.Select(i => i.Product.Category).OrderBy(i => i.Id),
                Is.EqualTo(ExpectedProductCategories.Where(i => i.Id == 1 || i.Id == 2))
                    .Using(new ProductCategoryEqualityComparer()),
                message: "GetByIdWithDetailsAsync method doesnt't return included entities");

            Assert.That(receipt.Customer,
                Is.EqualTo(ExpectedCustomers.FirstOrDefault(i => i.Id == expected!.CustomerId))
                    .Using(new CustomerEqualityComparer()),
                message: "GetByIdWithDetailsAsync method doesnt't return included entities");
        });
    }

    [Test]
    public async Task GetAllWithDetailsAsync_WhenCalled_ReturnsAllReceiptsWithIncludedEntities()
    {
        await using var context = new TradeMarketDbContext(UnitTestDataHelper.GetUnitTestDbOptions());
        ...
        Assert.Multiple(() =>
        {
            Assert.That(receipts,
                Is.EqualTo(ExpectedReceipts).Using(new ReceiptEqualityComparer()),
                message: "GetAllWithDetailsAsync method works incorrect");

            Assert.That(receipts.SelectMany(i => i.ReceiptDetails!).OrderBy(i => i.Id),
                Is.EqualTo(ExpectedReceiptsDetails).Using(new ReceiptDetailEqualityComparer()),
                message: "GetAllWithDetailsAsync method doesnt't return included entities");

            Assert.That(receipts.SelectMany(i => i.ReceiptDetails!).Select(i => i.Product).Distinct().OrderBy(i => i.Id),
                Is.EqualTo(ExpectedProducts).Using(new ProductEqualityComparer()),
                message: "GetAllWithDetailsAsync method doesnt't return included entities");

            Assert.That(
                receipts.SelectMany(i => i.ReceiptDetails!).Select(i => i.Product.Category).Distinct().OrderBy(i => i.Id),
                Is.EqualTo(ExpectedProductCategories).Using(new ProductCategoryEqualityComparer()),
                message: "GetAllWithDetailsAsync method doesnt't return included entities");

            Assert.That(receipts.Select(i => i.Customer).Distinct().OrderBy(i => i.Id),
                Is.EqualTo(ExpectedCustomers).Using(new CustomerEqualityComparer()),
                message: "GetAllWithDetailsAsync method doesnt't return included entities");
        });
    }
