...
Building the basic infrastructure for the `SportsStore` application. This step focuses on creating the project structure, basic MVC setup, and initial configuration.
...
```bash
$ git clone https://gitlab.com/your-username/sports-store-application.git
...
**Note:** The project already includes a comprehensive `.editorconfig` file that automatically configures code analysis rules, formatting, and naming conventions.
...
// NEW: Enable static files middleware
app.UseStaticFiles();
  
// NEW: Configure default routing
...
$ git add .
$ git diff --staged
$ git commit -m "feat: set up SportsStore project with MVC folders and basic configuration"
...
```bash
$ git push --set-upstream origin step-1

```
- Switch to the `main` branch and do a fast-forward merge according to changes from the `step-1` branch.
...
$ git push
```
- Go to the `Sports Store Application. Step 2.` (branch `step-2`).
...
1. [Get started with ASP.NET Core MVC](https://docs.microsoft.com/en-us/aspnetcore/tutorials/first-mvc-app/start-mvc?view=aspnetcore-8.0&tabs=visual-studio)
...
<details><summary>[Adam Freeman: Pro ASP.NET Core 7, Tenth Edition](https://www.amazon.com/Pro-ASP-NET-Core-7-Tenth/dp/1633437825). 
</summary> 
